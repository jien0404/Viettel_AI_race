...
def split_pdf(pdf_path: str, output_dir: str, pages_per_chunk: int = 20) -> List[str]:
    """
    Chia file PDF lớn thành nhiều phần, mỗi phần tối đa `pages_per_chunk` trang.
    Trả về danh sách đường dẫn các file PDF nhỏ.
    """
    ...
    for start in range(0, total_pages, pages_per_chunk):
        ...
        chunk_path = os.path.join(
            output_dir,
            f"{os.path.splitext(os.path.basename(pdf_path))[0]}_part_{start//pages_per_chunk+1}.pdf"
        )
        with open(chunk_path, "wb") as f:
            writer.write(f)
        chunks.append(chunk_path)
    
    print(f"📄 Đã chia {pdf_path} thành {len(chunks)} phần (mỗi phần ~{pages_per_chunk} trang).")
    return chunks


def merge_markdowns(md_paths: List[str], output_path: str):
    """Gộp nhiều file Markdown lại thành một file duy nhất."""
    with open(output_path, "w", encoding="utf-8") as out:
        for md in md_paths:
            with open(md, "r", encoding="utf-8") as f:
                content = f.read().strip()
            out.write(content + "\n\n")
    print(f"📜 Đã gộp {len(md_paths)} phần thành: {output_path}")
...
def generate_answer_md(output_root, md_info_list):
    answer_path = os.path.join(output_root, "answer_1.md")
    ...
    for pdf in pdf_files:
        ...
        if os.path.isdir(expected_output_folder):
            ...
            if os.path.exists(main_md_path):
                ...
                print(f"⚠️ Thư mục tồn tại nhưng thiếu main.md")
            continue
        ...
        try:
            # BƯỚC 0️⃣: Chia nhỏ PDF để tránh OOM
            chunks_dir = os.path.join(output_root, "temp_chunks", pdf_name)
            pdf_chunks = split_pdf(pdf_path, chunks_dir, pages_per_chunk=25)
            merged_md_parts = []

            # BƯỚC 1️⃣: Xử lý từng phần
            for i, chunk_pdf in enumerate(pdf_chunks, start=1):
                print(f"🔹 Xử lý phần {i}/{len(pdf_chunks)}: {os.path.basename(chunk_pdf)}")

                auto_folder, chunk_name = run_mineru(chunk_pdf, output_root)

                # Chạy Camelot có điều kiện: nếu PDF quá lớn thì bỏ qua
                reader = PdfReader(chunk_pdf)
                if len(reader.pages) > 60:
                    print("⚠️ File con quá lớn, bỏ qua Camelot để tránh OOM.")
                    camelot_html_tables = []
                else:
                    camelot_html_tables = extract_and_process_tables_with_camelot(chunk_pdf)

                output_folder, main_md, pdf_title = process_auto_folder(
                    auto_folder, chunk_name, camelot_html_tables
                )
                if main_md:
                    merged_md_parts.append(main_md)

                # Dọn VRAM giữa các phần
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
                    print("🧹 Đã giải phóng VRAM sau phần", i)
                time.sleep(1)

            # BƯỚC 2️⃣: Gộp tất cả phần lại
            final_output_folder = os.path.join(output_root, pdf_name)
            os.makedirs(final_output_folder, exist_ok=True)
            final_md_path = os.path.join(final_output_folder, "main.md")
            merge_markdowns(merged_md_parts, final_md_path)

            md_info_list.append((final_md_path, pdf_title))
            ...
            print(f"❌ Lỗi nghiêm trọng khi xử lý {pdf_name}: {e}")
            if os.path.isdir(expected_output_folder):
                shutil.rmtree(expected_output_folder, ignore_errors=True)
                print(f"   -> Đã dọn dẹp thư mục lỗi: {expected_output_folder}")
            continue

    # BƯỚC 3️⃣: Tạo file answer.md
    if md_info_list:
